a# CMPT 120 Intro to Programming
# Chapter 11 lab: Lists and Error Handling
# Author: CarLIsaac Nicolas
# Created: 2018 11 2
board = [[0,0,0],[0,0,0],[0,0,0]]  # TODO replace this with a list containing 3 elements, each one being a list of 3 zeroes
...
def printRow(row):
    ...
    for i in range(3):
        print(" " + symbol[row[i]] + " " + "|", end="")  # this will keep everything on the same line (no line return after printing)
    print(end='\n')
    print("+-----------+")# this will start a new line on next print.
...
def printBoard(board):
    ...
    for i in range(3):
        printRow(board[i])  # Call "printRow()" but pass the list element for the current row as a parameter instead of "(0,0,0"
